# data ingestion: load data from source, preprocess it, split into train and test, save to disk
import numpy as np
import pandas as pd
...
def main():
    try:
        ...
        df = load_data(data_url='https://raw.githubusercontent.com/aashu-0/MLOps_Learning_Project/refs/heads/main/notebooks/data.csv')
        # s3 = s3_connection.s3_operations("bucket-name", "accesskey", "secretkey")
        # df = s3.fetch_file_from_s3("data.csv")

        final_df = preprocess_data(df)
        train_data, test_data = train_test_split(final_df, test_size=test_size, random_state=42)
        save_data(train_data, test_data, data_path='./dataset')
    except Exception as e:
